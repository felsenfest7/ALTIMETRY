...
def corr_ssh_plot(df, title, mss):
    """
    ...
    fig, ax = plt.subplots()
    ax.yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
    ax.plot_date(dff["Tarih"], dff["SSH"], "#0d88e6", label="SSH Verileri")
...
def harmonik_analiz2(df, title):
    ...
    plot = corr_ssh_plot(df_merged, f"{title} Altimetri Verileri", mss)


haa = harmonik_analiz2(wish, "Antalya X-TRACK")
print(haa)
