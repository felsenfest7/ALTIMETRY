...
wish = df2newdf_xtrack(filtered, "1980-08-01")
...
def corr_ssh_plot(df, title, mss, trend):
    ...
    fig, ax = plt.subplots()
    #ax.yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
    ax.plot_date(dff["Tarih"], dff["SSH_ilk"], "#FFBF00", label="Model Öncesi SSH Verileri")
    ax.plot_date(dff["Tarih"], dff["SSH_model"], "#0d88e6", label="Model Sonrası SSH Verileri")

    ax.axhline(y = mss, c = "red", label = "MSS")
    plt.text(2018, 25.47, trend, fontsize=10)
